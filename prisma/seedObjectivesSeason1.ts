// prisma/seedObjectivesSeason1.ts
import { PrismaClient } from "@prisma/client";

const prisma = new PrismaClient();

const SEASON_CODE = "S1";
...
const season1Objectives: SeasonObjectiveConfig[] = [
...
const season1ObjectiveSets: SeasonObjectiveSetConfig[] = [
  {
...
async function main() {
  // Upsert objectives
  for (const o of season1Objectives) {
    await prisma.objectiveTemplate.upsert({
      where: { code: o.code },
      create: {
        code: o.code,
        name: o.name,
        ...
        seasonCode: o.seasonCode ?? null,
        sortOrder: o.sortOrder ?? 0,
        isRepeatable: o.isRepeatable ?? false,
  ...
  // Upsert sets and link objectives
  for (const set of season1ObjectiveSets) {
    const createdSet = await prisma.objectiveSet.upsert({
      where: { code: set.code },
      create: {
        code: set.code,
        title: set.title,
        description: set.description,
        seasonCode: set.seasonCode,
    ...
    const objectiveTemplates = await prisma.objectiveTemplate.findMany({
      where: { code: { in: set.objectiveCodes } },
    });

    await prisma.objectiveSetObjective.deleteMany({
      where: { objectiveSetId: createdSet.id },
    });

    for (const obj of objectiveTemplates) {
      await prisma.objectiveSetObjective.create({
        data: {
          objectiveSetId: createdSet.id,
          objectiveId: obj.id,
        },
      });
    }
  ...
  console.log("Season 1 objectives + sets seeded/updated.");
}
